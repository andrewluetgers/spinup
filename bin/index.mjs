...
import { inquireRepositoryDetails, promptForGitHubToken } from "../src/cli/prompts.mjs"
...
function getRepositoryTemplateChoices() {
    ...
    const templatesPath = path.join(scriptPath, '../templates')
...
async function copyTemplateFiles(templateName, destinationPath) {
    const scriptPath = path.dirname(new URL(import.meta.url).pathname)
    const templatePath = path.join(scriptPath, '../templates', templateName)
